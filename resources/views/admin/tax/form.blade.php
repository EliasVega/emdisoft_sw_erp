<div class="box-body row">
    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
        <div class="form-group">
            ...
            <input type="text" name="name" value="{{ old('name', $tax->name ?? '') }}" class="form-control" placeholder="Tributo">
        </div>
    </div>
    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
        <div class="form-group">
            <label for="description">Descripcion</label>
            <input type="text" name="description" value="{{ old('description', $tax->description ?? '') }}" class="form-control" placeholder="Descripcion">
        </div>
    </div>
    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="select">
            <select id="tax_type_id" name="tax_type_id" class="form-control selectpicker" data-live-search="true" required>
                <option {{ old('tax_type_id', $tax->tax_type_id ?? '') == '' ? "selected" : "" }} disabled>Seleccionar Impuesto</option>
                @foreach($taxTypes as $taxType)
                    @if(old('tax_type_id_id', $tax->tax_type_id ?? '') == $taxType->id)
    ...
    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="select">
            <select id="percentage_id" name="percentage_id" class="form-control selectpicker" data-live-search="true" required>
                <option {{ old('percentage_id', $tax->percentage_id ?? '') == '' ? "selected" : "" }} disabled>Seleccionar porcentage</option>
                @foreach($percentages as $percentage)
                    @if(old('tax_id', $tax->percentage_id ?? '') == $percentage->id)
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mt-3">
        <div class="form-group">
            ...
            <a href="{{url('tax')}}" class="btn btn-blueGrad"><i class="fa fa-window-close"></i>&nbsp; Cancelar</a>
        </div>
    </div>
</div>
