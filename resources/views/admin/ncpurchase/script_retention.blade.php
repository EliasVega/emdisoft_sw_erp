        ...
        tax.forEach((value, i) => {
            if (value['tax_value'] > 0) {
                ...
                if(taxName!='', taxValue!="" && taxValue>0){
                    ...
                    var rowretention= '<tr class="selected" id="rowretention'+contRetention+'"><td><button type="button" class="btn btn-danger btn-sm" onclick="deleteRetention('+contRetention+');"><i class="fa fa-times"></i></button></td><td><input type="hidden" name="tax[]" value="'+contRetention+'">'+contRetention+'</td><td><input type="hidden" name="company_tax_id[]" value="'+taxTypeid+'">'+taxTypeid+'</td><td><input type="hidden" name="company_tax_id[]" value="'+taxId+'">'+taxName+'</td><td> $'+parseFloat(totalRetention[contRetention]).toFixed(2)+'</td></tr>';
                    contRetention++;
        ...
        tax.forEach((value, i) => {
            if (value['tax_value'] > 0) {
                ...
                if(taxName!='', taxId!=''){
                    ...
                    //totalRetention[contRetention] = taxValue;
                    total_retention = parseFloat(total_retention)  + parseFloat(totalRetention[contRetention]);
                    var rowretention= '<tr class="selected" id="rowretention'+contRetention+'"><td><button type="button" class="btn btn-danger btn-sm" onclick="deleterow('+contRetention+');"><i class="fa fa-times"></i></button></td><td><input type="hidden" name="tax[]" value="'+contRetention+'">'+contRetention+'</td><td><input type="hidden" name="tax_type_id[]" value="'+taxTypeid+'">'+taxTypeid+'</td><td><input type="hidden" name="company_tax_id[]" value="'+taxId+'">'+taxName+'</td><td> $'+parseFloat(totalRetention[contRetention]).toFixed(2)+'</td></tr>';
                    contRetention++;
