...
if (! function_exists('pays')) {
    function pays($request, $document, $typeDocument)
    {
        ...
        if ($indicator->pos == 'off') {
                ...
                switch($typeDocument) {
                    ...
                    case 'invoice':
                        if ($indicator->pos == 'on') {
                            ...
                            $paymentReturn->payment = $request->payment;
                            $paymentReturn->return = $return;
                            $paymentReturn->invoice_id = $document->id;
                            $paymentReturn->save();
                        }
                    break;
                    default:
            ...
            //metodo para actualizar la caja
            $cashRegister->in_invoice_cash += $payment;
            $cashRegister->cash_in_total += $payment;

            $cashRegister->in_invoice += $payment;
            $cashRegister->in_total += $payment;
            $cashRegister->update();
